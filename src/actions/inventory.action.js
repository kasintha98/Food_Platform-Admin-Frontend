...
export const getInventoryUOM = (restaurantId) => { // STATIC-ROO1
  return async (dispatch) => {
    try {
...
export const getInventoryCategories = (restaurantId) => { // STATIC-ROO1
  return async (dispatch) => {
    try {
...
export const getInventorySubCategories = (restaurantId) => { // STATIC-ROO1
  return async (dispatch) => {
    try {
...
export const getInventoryPurchaseCategory = (restaurantId) => { // STATIC-ROO1
  return async (dispatch) => {
    try {
      dispatch({
...
export const saveUpdatePurchaseOrder = (item, isPullPo,userObj) => {
  return async (dispatch) => {
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        if (isPullPo) {
          dispatch(getSubmittedRecievedPurchaseOrders(userObj.restaurantId,userObj.storeId));
        }
        return res.data;
      } else {
        dispatch({
...
export const getSubmittedRecievedPurchaseOrders = (restaurantId,storeId) => {
  return async (dispatch) => {
    try {
      dispatch({ type: inventoryConstants.GET_CLOSED_PURCHASE_ORDERS_REQUEST });
